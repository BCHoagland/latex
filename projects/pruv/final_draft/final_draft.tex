...
\begin{proof}
	...
	We'll also need a bit of new notation. Let $N$ denote the number of vertices in our group of $m$ whose cluster size is $\leq K$, and let $P_{\leq K} = P_1 + \cdots + P_{K}$. Then for $s>K$, we can partition $\phi(s)$ and use the fact that it acts exactly like \ER if all $m$ of the vertices in the group come from clusters of size $> K$.
	\begin{align*}
	...
	\begin{align*}
		\phi(s) &= \cancel{\P{ \kappa = s \;|\; N = 0}}\P{ N=0 } + \P{ \kappa = s \;|\; N>0 } \P{ N>0 } \\
			&= \P{ \kappa = s \;|\; N>0 } \P{ N>0 }.
	\end{align*}
	This allows us to sum over all $\phi(s)$ and thus calculate $\ang{1}_{\phi}$.
	\begin{align*}
		\sum_{s} \phi(s) &= \left[ \sum_{s > K} \phi(s) \right] + \left[ \sum_{ s \leq K}\phi(s) \right] \\
				 &= \left[ \sum_{s>K}(1-P_{\leq K})^{m-1} \right] + \left[ \sum_{s} \P{ \kappa=s\;|\; N>0 }\P{ N>0 } \right] \\
				 &= (1-P_{\leq K})^{m-1}(1-S-P_{\leq K}) + (1-(1-P_{\leq K})^{m}) \sum_{s} \P{ \kappa=s\;|\; N>0 } \\
				 &= (1- P_{\leq K})^{m-1} \left[ 1 - S - \xi + (\xi-1)P_{\leq K} \right] + \xi,
	\end{align*}
	where $\xi = \sum_{s} \P{ \kappa=s\;|\; N>0 }$ is the probability that the chosen vertex's cluster is finite given that there is at least 1 finite cluster to choose from in the group. In scaling form, the first term above will be dominated by the smallest critical exponent since we care about $\delta$ as it approaches 0. After multiplying everything out, the term with the lowest exponent will be $S$ (with critical exponent $\beta$).

	We will now induct on the group size $m$ to show that $\xi \sim \beta$. When $m=1$,
	\[
	...
	\]
	as desired. Thus as $\delta \to 0$, the scaling form of $\ang{1}_{\phi}=\sum_{s}\phi(s)$ is
	\[
	\ang{1}_{\phi} \sim \delta^{\beta} = \delta^{-\gamma_{1}},
	\] 
	where the last equality follows from \Cref{crit-exp}.

	By definition, this fixes the standard induced order map of this rule. Since our computation of the critical exponents of 2-choice rules relied only on the standard induced order maps, it follows that our rule has the same critical exponents as \ER.
\end{proof}

A crucial relation used in the previous theorem was the fact that
\[
\xi = \sum_{s} \P{ \kappa=s\;|\; N>0 }
\] is dominated by other terms as we approach criticality. For many bounded size 2-choice rules, more can be said of this term. For example, the \BF rule is a bounded size rule with threshold $K=1$, and $\xi_{BF}= 1$ since it always prioritizes connecting isolated vertices. It is already intuitively clear that the \BF rule cannot prioritize isolated clusters (more generally, the clusters below its size threshold) any further, but this classification gives that statement rigorous meaning.
