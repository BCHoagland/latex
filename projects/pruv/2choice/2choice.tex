...
%\importStyles{modern}{rainbow}{lined}
...
\warn{NEED TO REDO THIS SECTION BASED ON GENERAL THINGS INSTEAD, AND BE VERY CLEAR ABOUT WHAT ALL THE DIFFERENT COEFFICIENTS ARE.}
...
\begin{proof}
	...
	\]
	Suppose $F_i(\beta)$ is the induced coefficient map of $\zeta_i(S)$ \warn{(Check to make sure that the two terms this applies to actually dominate)}. Then there are two dominating terms, and both have order $F_1(\beta) + F_2(\beta) - 1/\sigma$.
\end{proof}
...
\begin{ex}[]
	...
	\end{align*}
	By Theorem \ref{thrm:m-beta-0}, $\var_i(s) \to \delta^{-2}$ for both $i$. \warn{(I wonder if the exponent is actually $\tau$)}
\end{ex}
