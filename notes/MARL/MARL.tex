...
\warn{Think I want to change the notation to be $\mathcal{B}$ instead, to make later math easier to read}
\begin{cor}
...
\begin{proof}
	...
	To show (2), we use \Cref{bellmanoperators}'s result that the Bellman operator $\mathcal{Q}^{*}$ is a $\gamma$-contraction in the max norm with unique fixed point $Q^{*}$:
	\begin{align*}
