...
%-------------------
\subsection{Basic Commutative Diagrams}
...
Fix a sequence $0\to A \stackrel{f}{\to } B\stackrel{g}{\to } C\to 0$, then TFAE:
\begin{enumerate}
	\item it splits on the left;
	\item it splits on the right;
	\item it splits.
\end{enumerate}
\end{thrm}
\begin{proof}
...
\begin{note}[]
	Everything below is implicitly assumed to be using $R$-modules and $R$-morphisms.
\end{note}
\begin{prop}
