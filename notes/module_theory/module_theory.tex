...
\end{thrm}
\begin{proof}
	This is essentially just using the universal properties of free modules and coproducts.Fix $\alpha$, and let $M$ and $f_{\alpha}:S_{\alpha}\to M$ be arbitrary.
	\[
		\begin{tikzcd}
...
S_\alpha \arrow[rrd, "f_\alpha"', bend right] \arrow[u, "i_\alpha", hook] \arrow[r, hook, shift left] & \bigsqcup_\alpha S_\alpha \arrow[u, "i", hook] \arrow[rd, "g" description, dotted] \arrow[l, "\pi_\alpha", two heads, shift left] &   &                                              &  &   \\
                                                                                                      &                                                                                                                                   & M &                                              &  & M
\end{tikzcd}
	\] The diagram's got a lot going on, but it's straightforward. All four inclusions and the one projection are the natural ones. $f$ comes from the universal property of free modules, so $fi_{\alpha}=f_{\alpha}$. $\phi$ comes from the universal property of the coproduct, so $\phi j_{\alpha}=f$. We define $g$ by $g|_{S_{\alpha}} = f_\alpha \pi_{\alpha}$.
