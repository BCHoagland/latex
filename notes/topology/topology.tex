...
\begin{prop}
	\begin{enumerate}
		...
		\item Closed subsets of compact spaces are closed.
	\end{enumerate}
\end{prop}
...
\begin{thrm}[]
If $X$ is a compact metric space, then every sequence has a convergence subsequence.
\end{thrm}

\begin{thrm}[Lebesgue Number]
...
\[
	F(x,t) = (1-t)f(x)+tg(x).
\] 
If $X$ is convex, then the straight line homotopy can be used to show that any two maps are homotopic.
\end{ex}
...
If we have a continuous map
\begin{align*}
	f:X&\to Y\\
	x&\mapsto y,
\end{align*}
then this induces a homomorphism between fundamental groups
\begin{align*}
...
\end{cor}
{\color{red}Why? Also show that the converse isn't necessarily true.}
